...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        ../transformer/utils/split_scp.pl "${key_file}" ${split_scps}
        _opts=
        if [ -n "${inference_config}" ]; then
            _opts+="--config ${inference_config} "
        fi
        ../transformer/${infer_cmd} --gpu "${_ngpu}" --max-jobs-run "${_nj}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
            python3 -m funasr.bin.asr_inference_launch \
                --batch_size 1 \
                --ngpu "${_ngpu}" \
                --njob ${njob} \
        ...
        python3 utiles/proce_text.py ${_dir}/text ${_dir}/text.proc
        python3 utiles/proce_text.py ${_data}/text ${_data}/text.proc
        python3 utiles/compute_wer.py ${_data}/text.proc ${_dir}/text.proc ${_dir}/text.cer
